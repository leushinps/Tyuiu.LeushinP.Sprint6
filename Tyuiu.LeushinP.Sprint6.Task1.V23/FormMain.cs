...
namespace Tyuiu.LeushinP.Sprint6.Task1.V23
{
    public partial class FormMain : Form
    {
        ...
        private void buttonExecute_LPS_Click(object sender, EventArgs e)
        {
            DataService ds = new DataService();
            try
            {
                ...
                textOutput_LPS.Text = "";
                textOutput_LPS.AppendText("+----------+----------+" + Environment.NewLine);
                textOutput_LPS.AppendText("|    X     +   f(x)   |" + Environment.NewLine);
                textOutput_LPS.AppendText("+----------+----------+" + Environment.NewLine);

                for (int i = 0; i <= len - 1; i++)
                {
                    strLine = String.Format("|{0,5:d}     |  {1, 5:f2}   | ", startStep, valueArray[i]);
                    textOutput_LPS.AppendText(strLine + Environment.NewLine);
                    startStep++;
                }
        ...
        private void buttonAbout_LPS_Click(object sender, EventArgs e)
        {
